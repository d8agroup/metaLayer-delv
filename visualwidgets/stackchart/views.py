...
def widget_data():
    return { 'type':type, 'display_name':'Stack Chart' }

def generate_unconfigured_config(config, collection_id):
    return_data = {
        'id':urllib.quote(md5('%s' % random.random()).hexdigest()),
        'type':type, 
        'display_name':'Stack Chart',
        'config':{
            'configured':True,
            'active_chart_type':'Stacked Sentiment Chart',
            'chart_types':{
                'Stacked Sentiment Chart':{
                    'type':'native',
                    'module':'visualwidgets.stackchart.views',
    ...
    for views in [my_import('%s.views' % name) for name in settings.INSTALLED_APPS if re.search(r'actionwidgets\.', name)]:
        try:
            function = getattr(views, 'chart_stackchart_configuration')
...
def chart_source_type(request, collection_id, content, visual_id):
    ...
    url = '/widget/visualwidgets/stackchart/chart_source_type_js?collection_id=%s&visual_id=%s' % (collection_id, visual_id)
    visual_data = {
        'id':visual_id,
        'type':visual['type'],
        'url':url,
    ...
    """
        ...
        return render_to_response('stackchart_configure.html', { 'visual':visual, 'collection_id':collection_id })
    """
...
def chart_source_type_js(request):
    ...
    """
    return render_to_response('render_stackchart.js', { 'id':visual_id })
